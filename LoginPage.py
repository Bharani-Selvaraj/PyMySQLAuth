...
def forget_pass():
    def change_password():
        if user_entry.get()=='' or newpass_entry.get()=='' or confirmpass_entry.get()=='':
            ...
            con=pymysql.connect(host='localhost',user='root',password='Abi@2000',database='userdata')
            mycursor=con.cursor()
            ...
            if row==None:
                ...
                con.commit()
                con.close()
                messagebox.showinfo('Success','Password is reset, please login with newpassword',parent=window)
                window.destroy()
...
def login_user():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        try:
           con=pymysql.connect(host='localhost',user='root',password='Abi@2000')
           mycursor=con.cursor()
        except:
...
forgetButton=Button(login_window,text='Forget Password?',bd=0,
                 bg='white',activebackground='white',cursor='hand2',font=('Microsoft Yahei UI light',9,'bold'),
...
googleLabel=Label(login_window,image=google_logo,bg='white',cursor='hand2')
googleLabel.place(x=690,y=440)

twitter_logo=PhotoImage(file='twitter.png')
twitterLabel=Label(login_window,image=twitter_logo,bg='white',cursor='hand2')
...
newaccountButton.place(x=735,y=500)
